...
class InoS3UploadString(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            description="Saves a string as a file (txt/json/ini) locally then uploads it to S3.",
            is_output_node=True,
            inputs=[
    ...
    @classmethod
    async def execute(cls, execute, enabled, string, save_as, s3_key, parent_folder, folder, filename, s3_config=None, date_time_as_name=False) -> io.NodeOutput:
        if not enabled:
        ...
        full_output_folder, file_prefix, counter, subfolder, _ = folder_paths.get_save_image_path(filename, abs_path, 0, 0)
        ...
        if not save_file["success"]:
            return io.NodeOutput(string, False, save_file["msg"], rel_path, abs_path, "", "")

        s3_instance = S3Helper.get_instance(s3_config)
        if ino_is_err(s3_instance):
            return io.NodeOutput(string, False, s3_instance["msg"], rel_path, abs_path, "", "")
        ...
        if not s3_result["success"]:
            return io.NodeOutput(string, False, s3_result["msg"], rel_path, abs_path, "", "")

        return io.NodeOutput(string, True, "Success", rel_path, abs_path, file_w_ext, s3_full_key)
